...
void getData() {
  if (mode == 0) {
    String sensorData[8] = {
      string(gas.value()), String(pres.value()),      // Get the humidity data as a value
      String(bsec.iaq_s()), String(bsec.b_voc_eq()),  // Get the pressure sensor value
      String(bsec.co2_eq()), String(bsec.comp_t()),   // Get the VOC equivalent value from BSEC
      String(bsec.comp_h()), String(bsec.accuracy())  // Get the accuracy level from BSEC
    };
    int size = 8;
