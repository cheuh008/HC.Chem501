...
def plot(dfs: dict, channels: dict, folder: str = "plots"):
    ...
    for count, (name, df) in enumerate(dfs.items()):                                     # Iterate through channels and their data
        ...
        time_data = time_data.dt.strftime("%d %H:%M:%S")               # Extract the time (HH:MM:SS) from each datetime object
        ...
        for i, label in enumerate(labels):                          # Iterate through each label
            ...
            custom_ticks = [time_data.iloc[0], time_data.iloc[len(time_data) // 2], time_data.iloc[-1]]
            ax.set_xticks(custom_ticks)                             # Set x-axis ticks to only display the first, middle, and last times
            #ax.set_xticklabels([t.strftime("%Y-%m-%d %H:%M:%S") for t in custom_ticks], fontsize=8)
# ========= # ==============================================================================================================================
